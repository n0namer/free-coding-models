...
function requiresAtomicStream(body, structuredContract) {
...
class RouterRuntime {
  ...
  statusPayload() {
    ...
    return {
      ...
      effectiveProviderCount: effectiveProviders.size,
      emergencyProviderCount: Math.max(0, effectiveProviders.size - activeProviders.size),
  ...
  scheduleProbeLoop() {
    ...
    if (this.probeTimer && this.probeScheduleMode === probeRouter.probeMode && this.probeScheduleInterval === probeInterval) return
  ...
  async routeRequest({ req, res, body, setName, requestId }) {
    ...
    const structuredContract = buildStructuredOutputContract(body)
    if (!structuredContract.ok) {
      sendError(res, 400, structuredContract.error, 'invalid_request_error', 'invalid_json_schema', requestId)
      return
    }
    ...
    try {
      ...
      if (url.pathname.startsWith('/api/key/')) {
        ...
        if (req.method === 'GET') {
          ...
          const rawKey = this.getConfiguredApiKeyForProvider(providerKey)
