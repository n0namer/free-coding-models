...
## 2026-09-08 — fast provider-wide failures must not exhaust later-provider opportunity

- **Symptom**: Production stayed process-healthy and retained usable routes, yet an authenticated eight-turn cadence returned client-visible `All routed models failed` when several early providers failed quickly with provider-wide 429/auth-domain errors.
- **Cause**: The global bounded attempt budget counted those fast provider-wide skips the same as timeout/transport/5xx attempts. Multiple early quota/auth failures could consume all four attempts (`maxRetries=3`) before a later healthy provider was tried.
- **Fix**: Preserve provider-wide blocking semantics for 429/auth, but extend only the per-request candidate scan allowance for each fast provider-wide skip; keep timeout/transport/retryable 5xx on the ordinary retry budget and cap total scanning at `candidates.length`.
- **Prevention**: Regression-test a small set where `maxRetries=1`, provider 1 returns 429, provider 2 returns 429, and provider 3 succeeds. The request must reach provider 3 without increasing the canonical retry budget.
- **Verification**: The container-first live patch passed router syntax, the full live `npm test`, same-container reload, and an authenticated DEV canary. Product-code write-back reached canonical head `fae89d7b4a91090c3a61bd361f2556315a830f0a`; exact canonical regression-test write-back is separately blocked by Coding Station `ENOSPC` and must not be inferred PASS.
