      ...
      await withMockProvider(() => ({
        ...
        await withSourceUrls({ groq: primary.url, nvidia: fallback.url }, async () => {
          await withRouter(async (baseUrl) => {
            ...
            assert.equal(primary.requests.length, 2, 'full invalid response + failed same-candidate repair fragment')
      ...
      await withMockProvider(() => ({
        ...
        await withSourceUrls({ groq: primary.url, nvidia: fallback.url }, async () => {
          await withRouter(async (baseUrl) => {
            ...
            assert.equal(primary.requests.length, 2, 'primary full attempt + failed repair fragment')
            assert.equal(fallback.requests.length, 2, 'fallback full attempt + failed repair fragment')
