...
11. **Next bounded move.** Keep the CURRENT live FCM as the control because its functional gate is DONE. Use the first naturally occurring downstream multi-step SWE/OpenCode/AgentField execution as observational confidence only; if it completes without an FCM-layer error, record that evidence and stop. If it fails, reopen FCM only when the failure is evidenced at the broker boundary. Batch 7 exact-source/upstream convergence resumes separately when writable workspace capacity exists; do not mutate or redeploy the working runtime merely to clean up source identity.
