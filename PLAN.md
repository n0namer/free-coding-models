...
- The previous live router corruption is **recovered** and the current probe-scheduler fix is applied directly in the same permanent container. CURRENT `/app/src/core/router-daemon.js` is 4,470 lines / 196,719 bytes, SHA-256 `353e6726dbfa6349d207b915ffca968121096470cf0a9377d567e76d669febcf`; `/app/src/core/schema-normalizer.js` is 567 lines / 32,489 bytes. The old runtime still embeds structured-output adaptation differently from canonical Git, so byte-for-byte deployed source identity remains intentionally marked drifted.
- CURRENT live test owners are `/app/test/test.js` 6,996 lines / 296,803 bytes, `/app/test/probe-cache.test.js` 601 lines / 25,221 bytes, and `/app/test/router-client-auth.test.js` 808 lines / 37,341 bytes. The scheduler anti-starvation regression is present and focused PASS in the live probe tests; these old-image test files are not assumed byte-identical to current Git.
- Final live-container package gate after the scheduler fix: `npm test` discovered 832 tests, 830 passed, 0 failed, 2 intentional live-canary skips, 159 suites, ~18.5 s. Direct live canaries with runtime auth intact then returned 2/2 PASS (`plain-text`, `json_schema`).
...
4. **PASS — live package suite after cadence/failure-domain fix.** Final `npm test` PASS: 838 tests discovered, 835 passed, 0 failed, 3 intentional skips, 160 suites, ~18.8 s. Deterministic regressions cover unchanged probe scheduling, provider-level 429 skipping same-provider siblings, route-local transport/5xx failure trying one sibling before crossing provider domains, structured-output failover, and the earlier scheduler/circuit invariants. Syntax checks for the live router are PASS.
