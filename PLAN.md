...
- CURRENT isolated `outreach-quality` is a four-route contract-compatible fallback set: `codestral/codestral-2508` → `groq/openai/gpt-oss-120b` → `groq/openai/gpt-oss-20b` → `llm7/codestral-latest`. Contract-invalid candidates were excluded; the isolated set completed a real 36-item consumer replay while preserving `fast-coding` membership/order.
- Fresh 2026-09-09 inventory: `/api/models` exposes 206 catalog models; 116 model routes are credentialed; only 7 are CURRENT `status=up`: `googleai/gemini-3-flash-preview`, `googleai/gemini-3.1-flash-lite`, `codestral/codestral-2508`, `zai/zai/glm-4.5-flash`, `llm7/minimax-m2.7`, `gonka/MiniMaxAI/MiniMax-M2.7`, `gonka/deepseek-ai/DeepSeek-V4-Flash-0731`. Therefore the Outreach quality pool target is dynamic: include the maximum CURRENT production-contract-compatible routes, not a hard-coded 20–30 count.
- Fresh live code inspection proves the router has no model race/hedging primitive for ordinary JSON requests: `routeRequest()` iterates candidates sequentially (`for (const candidate of candidates)`); existing `Promise.race` usages implement timeouts/probes only. The current four-route `outreach-quality` is therefore ordinary ranked/failover routing, not race-first architecture.
- Course correction: `outreach-quality` must evolve into a broad dynamic pool of CURRENT contract-certified models, but Tier 1 is a fixed architectural invariant: `gonka/deepseek-ai/DeepSeek-V4-Flash-0731` and `gonka/MiniMaxAI/MiniMax-M2.7` are always priorities 1–2 and race/hedge each other for Outreach primary/recommendation traffic. Health/circuit state may temporarily skip an unavailable Gonka route, but scoring/refresh automation must never demote either Gonka route below Tier 1. Remaining certified routes provide scored/circuit-breaker fallback. A separate `outreach-judge` pool must prefer non-Gonka provider/model families for failure-domain and bias independence. Outreach semantic/business validation remains outside FCM; FCM must not embed Outreach-specific `message_id` or evidence logic. Models qualify for fallback/judge pools only through production-equivalent structured contract probes.
