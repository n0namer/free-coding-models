...
Status: ACTIVE — **FCM routing reprioritization is waiting on an operator control-plane repair that is now implemented and in DEV deployment/verification.** User-approved objective: durable tiers (1) Gonka first, (2) strongest proven free routes roughly DeepSeek-class, (3) paid RouterAI, (4) weaker free routes. Bounded health/circuit failover remains authoritative inside this durable preference policy; do not silently re-pin RouterAI to #1.
Last reconciled: 2026-09-12
Repository: `n0namer/free-coding-models`
Active working branch: `fix/fcm-long-stream-lifecycle`
Current deployed FCM product-code head in DEV and PROD remains `6e17a250cd34469a14d575a45c20d0f2f6f8cfab`; PROD is intentionally untouched until DEV proves the new policy. The obsolete routing owner remains `src/core/config.js`, where env-derived RouterAI is constructed at priority 1 and normalized ahead of configured routes. The DEV live-patch blocker was root-caused to a transport/bootstrap contract: `fcm-dev` is a Node live-patch target rooted at `/app`, but `/app/live-aci-node-helper.mjs` is absent. A temporary exact-id registry experiment proved that switching this target to the Python helper does not solve the lane and was fully rolled back; the temporary registry mutation privilege was then closed and independently re-probed as denied. The operator fix is therefore at the owning layer: VPS Terminal now has a bounded Node-helper transport fallback that activates only when the target-local helper is proven missing, while preserving the existing target-local path as default. The exact source delta was made container/source-first in the operator workbench, written back to `n0namer/vps-terminal` branch `archops/k4b-container-cleanup` at head `7ed1daa2824096e2624c0025f7ee9571fa78ff87`, and a DEV operator deployment is currently in progress. GitHub Actions for that head failed before job creation (`startup_failure`, zero jobs), while the existing workbench validation runner is independently blocked by a stale missing `memory-emitter.mjs` preload; neither is evidence of an application test failure. Acceptance for this control-plane repair is functional: the deployed operator must become healthy on the exact new source and `fileAction read` plus `node_check` against `fcm-dev` must succeed without placing helper files inside FCM.
...
- The CURRENT branch head is the commit containing this SoT; do not hard-code it here because every PLAN reconciliation advances that value. The latest live-verified product-code head is `fae89d7b4a91090c3a61bd361f2556315a830f0a`; later commits in this branch are PLAN/ERRORS operational reconciliation unless separately marked as live-verified product code.
...
- **Fresh FCM-only readback (2026-09-08).** DEV (`krhkfc6xjtreidxxbf8xdia3`, alias `fcm-dev-internal`) is `running:healthy`: 33 models, 9 configured / 9 effective providers, `autoHeal=true`, `requestsRouted=1`, `successCalls=1`, `errorCalls=0`, `inFlight=0`. PROD (`5llbaomtlttzsukhin6xgrgc`, alias `fcm-prod-internal`) is `running:healthy` on configured Git SHA `fae89d7b4a91090c3a61bd361f2556315a830f0a`: 33 models, 9 configured / 8 effective providers, `autoHeal=false`, `requestsRouted=7`, `inFlight=2`. Both expose `/config/config.json` as runtime config owner. Current provider/probe counts are transient health signals, not durable-set changes.
- **Scope correction.** External consumers are NON-TARGET and are not part of FCM acceptance or current DoD. Prior consumer/AgentField observations are historical context only and must not drive FCM mutations.
...
- The permanent runtime is healthy after the current container-first fixes. CURRENT `/app/src/core/router-daemon.js` is 4,552 lines / 200,586 bytes, SHA-256 `9a5b407869e9d7697ed9b6bcbabefb0523fa1642d4317c86ed608d18768db058`. It includes the live-verified request failure-domain fix and OpenAI-compatible `fcm:<set>` resolution/fail-closed edge hardening. The old runtime still embeds structured-output adaptation differently from canonical Git, so byte-for-byte deployed source identity remains intentionally marked drifted.
- CURRENT live test ownership still spans the old-image `test/test.js`, `test/probe-cache.test.js`, and live-only `test/router-client-auth.test.js`; these files are not assumed byte-identical to current Git. The named-set positive/negative regressions, request failure-domain regressions, and authenticated cadence canaries are green in this runtime.
- Final live-container package gate after the named-set edge hardening remains green: a fresh canonical `npm test` on 2026-09-07 exits `0`; the previously counted gate is 841 tests discovered, 837 passed, 0 failed, 4 intentional skips, 160 suites. Authenticated black-box acceptance includes ordinary text PASS, `json_schema` PASS, named-set fail-closed behavior, and repeated multi-turn reliability. In the latest FCM-only soak, three consecutive eight-turn reviewer-like cadences completed PASS (24/24 routed client requests, 0 runtime telemetry errors). CURRENT router-status readback after an additional fresh live plain-text canary now shows `requestsRouted=27`, `successCalls=27`, `errorCalls=0`, 9 effective providers, and 12/33 probe-fresh healthy routes (21 broken/hidden). The effective-provider count recovered from 8 to all 9 configured providers while the per-route probe pool continued to fluctuate, confirming that provider availability and route freshness must be interpreted separately. Recent daemon logs localize the degraded pool to recurring upstream HTTP 429 and probe timeouts on several secondary providers; Gonka/LLM7/Kilo/Mistral and some GoogleAI routes continue probing successfully. No `All routed models failed` occurrence is present in the current daemon log.
...
- Live black-box acceptance is PASS after the latest same-container reload: ordinary text and `response_format=json_schema` are 2/2 PASS through the current SWE-first routing order; no provider name is assumed as the acceptance criterion. A previous 32-token schema canary was a false negative caused by truncating reasoning/output; the acceptance canary uses a realistic 256-token budget.
...
- The earlier four red `router-json-schema.test.js` results were historical test-harness URL defects (`http://127.0.0.1${port}` missing `:`), not router behavior; that harness defect remains resolved and is not part of the current gate.
- SourceLoop/FVE journal is CURRENT and records the live-patch chain; the former gap is closed without changing FCM behavior: `fcm-dev` is registered for `live_patch` with a SourceLoop `writeback` mapping, and new journal rows now carry populated `writeback` + `capture_envelope`; forward and reverse `sourceLoopAction artifact` reads are proven PASS. SourceLoop artifact export itself is proven healthy on the mapped `vps-terminal-dev-gateway` target. CURRENT FCM container image tag `krhkfc6xjtreidxxbf8xdia3:cd64d76cb6e9c7ede9c7ce556b786e8732a4a81e` is an exact Git commit in this repository, so `container_image_tag_sha` is viable. The typed registry upsert then succeeded at revision `2`; the temporary one-target allowlist was restored exactly and a current-SHA no-write revision-3 probe returned `target_registry_scope_denied`, proving the privilege window closed. The reversible canary touched only a non-loaded test-file comment, restored the exact original SHA, and required no FCM reload/redeploy. SourceLoop future-capture readiness is therefore **PASS/CLOSED**; this does not resolve the separate deployed-source identity drift.
- Fresh exhaustive discovery on 2026-09-05 scanned 131 routeable catalog candidates backed by configured provider credentials. Exactly 33 candidates returned a fresh successful chat probe and were persisted into `fast-coding`; the other candidates were excluded rather than assumed healthy. The final set spans 9 provider identities: Gonka (2), LLM7 (3), Kilo (2), OpenRouter (7), Google AI (4), Mistral (3), Z.AI (2), OpenCode Zen (3), and Requesty (7). The set is ordered SWE-first (`sweScore` descending, tier only as a tie-break, then prior proven order). A generic chat probe proves route liveness, not `json_schema` capability; structured incompatibility remains fail-closed and route/role scoped by `structuredRouteBlocks`. A later anti-drift check found that the configured 30s balanced probe loop was being reset every 10s by config reload and therefore only startup probes were actually running. That scheduler starvation is now fixed live and written back canonically: unchanged mode/interval no longer resets the timer, while real schedule changes still do.

## Current Stage

**P2.2 ROUTERAI PRIMARY ROLLOUT — DEV DONE / PROD DEPLOYMENT UNRESOLVED.**

Active work is FCM-only. RouterAI is the user-approved permanent primary route: priority #1 in every set, with Gonka shifted to #2/#3 and ordinary bounded failover preserved. DEV is accepted on exact `6e17a250cd34469a14d575a45c20d0f2f6f8cfab`; PROD remains on the previous healthy `fae89d7...` runtime while force deployment `ztlptxssouuw6pjbl238zcwz` is unresolved. Do not start another PROD mutation or call rollout DONE until that queue row reaches a terminal state and the replacement container is independently verified.

CURRENT live runtime has progressive `json_schema` repair, a persisted 33-route `fast-coding` pool across 9 provider identities, and Gonka pinned at priorities 1/2. The flapping incident was traced to request-local failure-domain handling, not SWE runtime failure: a route-local timeout/transport/retryable 5xx on the priority-1 route could immediately block the whole provider and skip the priority-2 sibling, wasting the bounded four-attempt budget on other providers. The permanent `fcm-dev` container is now patched container-first so route-local provider failures receive exactly one untried sibling attempt before crossing provider domains; HTTP 429 remains provider-wide and crosses domains immediately because quota/key cooldown is shared, and auth failures remain provider-wide immediately. The global retry budget is unchanged: `maxRetries=3` means at most four model attempts. Deterministic P1.6 regressions for provider-wide 429 and route-local transport failure are PASS; router syntax is PASS; the same container was restarted without rebuild/redeploy and returned healthy with `activeSet=fast-coding`, `activeModelCount=33`, and `activeProviderCount=9`. The authoritative runtime config is `/config/config.json` and persists `maxRetries=3`, `requestTimeoutMs=60000`, and `streamStallTimeoutMs=30000`; the stale `/root/.free-coding-models.json` is not the daemon config SoT. Authenticated post-fix acceptance is now PASS: live plain text PASS, live `json_schema` PASS, and an eight-turn reviewer-like `fast-coding` cadence PASS. During that acceptance the broker delivered 10/10 successful client requests while recording two real priority-1 Gonka `http_429` upstream failures and transparently routing successful fallbacks. The specific `All routed models failed` cadence incident is therefore functionally closed in the live runtime.
...
- **P0 reliability boundary — PASS.** The original eight-turn reviewer-like acceptance delivered 10/10 successful client requests while real priority-1 Gonka 429 failures were absorbed by fallback. A fresh FCM-only repetition on 2026-09-07 then ran three consecutive eight-turn cadences for 24/24 additional successful routed requests with 0 telemetry errors. One request took ~20.8 s but remained within the configured 60 s request timeout. This directly exercises stable multi-turn use and, together with deterministic 429/failure-domain tests, covers the reported `All routed models failed` failure mode at the broker boundary without requiring a consumer integration.
- **Consumer integration — NON-GOAL for this FCM acceptance.** The authenticated multi-turn cadence already uses the real FCM OpenAI-compatible surface directly. A SWE/OpenCode/OpenClaw/AgentField binding is neither required nor useful as a gate for whether the broker itself works.
- **Consumer observations remain NON-TARGET.** Current OpenClaw agents are configured for direct Gonka rather than FCM, and the observed 90 s OpenClaw timeout was localized to that direct transport. AgentField discovery currently returns `Bad Gateway`. Neither condition changes the FCM verdict, and neither should trigger FCM code changes.
- **Post-fix recurrence check — PASS.** The current OpenClaw log contains no `All routed models failed for set: fast-coding` / FCM-endpoint signature, and the CURRENT FCM daemon log contains no `All routed models failed` occurrence. This is observational evidence only, but it is directionally consistent with the authenticated cadence PASS.
- **BMAD Test Design risk decision.** P0 broker recurrence is mitigated by deterministic failure-domain tests + authenticated cadence + recurrence-free current logs. The remaining observed timeout risk is a P1 consumer/direct-provider concern outside FCM scope. Exact-source identity and upstream convergence remain OPS/TECH maintenance risks for a future clean release, not functional runtime blockers.
...
- **Functional FCM closure is DONE; 2 release-hygiene tasks remain non-blocking:**
  1. **Exact-source full-suite evidence — ENVIRONMENT-GATED MAINTENANCE.** A transient Coding Station family-failover candidate was built for research, but router integration/full-suite validation never ran because dependencies were absent and `npm ci --ignore-scripts` failed with `ENOSPC`. That candidate is now reference-only under the latest container-first execution rule and must not be published/deployed as implementation. This is a validation-environment blocker, not an FCM runtime/test failure. Future durable code deltas must be proven first in `fcm-dev`, then written back to Git.
  2. **Exact deployed source identity — DRIFT CONFIRMED / NON-BLOCKING FOR CURRENT USE.** The permanent container is based on the older image/source lineage and is not byte-identical to current Git. Safe future convergence therefore requires a tested release boundary, not further in-place identity claims. This does not reopen the functional FCM gate.
- **SourceLoop readiness: PASS/CLOSED.** Runtime and canonical target registration now carry the provenance mapping, the temporary privilege window is proven closed, and forward/reverse artifact canaries passed with exact file restoration. Future live deltas are attributable; historical source-layout drift remains a separate release-convergence problem.
...
4. **PASS — live package suite after cadence + named-set edge hardening.** Final `npm test` PASS: 841 tests discovered, 837 passed, 0 failed, 4 intentional skips, 160 suites, ~18.5 s. Deterministic regressions cover unchanged probe scheduling, provider-level 429 skipping same-provider siblings, route-local transport/5xx failure trying one sibling before crossing provider domains, structured-output failover, positive `fcm:<set>` named-set routing, and fail-closed empty virtual-set selection. Syntax checks for the live router are PASS.
5. **PASS — authenticated post-fix FCM reliability cadence.** The persisted 33-route `fast-coding` set spans 9 provider identities and the Gonka-first order survives the same-container reload. Direct live plain text and `response_format=json_schema` canaries are PASS. The original acceptance delivered 10/10 successful client requests while two real priority-1 Gonka `http_429` failures were transparently recovered. The latest FCM-only soak then ran three consecutive eight-turn cadences for 24/24 additional successful requests with 0 telemetry errors. CURRENT status after an additional fresh live plain-text canary has 9 effective providers and 12/33 probe-fresh healthy routes (21 broken/hidden), with `requestsRouted=27`, `successCalls=27`, `errorCalls=0`; no `All routed models failed` escaped to the client. The degraded probe pool is therefore an operability signal to watch, not evidence that the current request path is failing.
6. **PASS — Gonka-pinned head + provider-aware recurring real-inference health routing.** The 33 persisted routes are explicitly headed by Gonka DeepSeek at priority 1 and Gonka MiniMax at priority 2; the remaining 31 routes preserve their prior relative SWE-ranked order. FCM does not run its own quality benchmark. `probeMode=balanced` persists at 120s and uses real chat completions. HTTP 429 pauses the provider/key domain using `Retry-After` when present, startup probes serialize siblings within each provider, periodic probes exclude OPEN routes until HALF_OPEN recovery and re-check provider pause at dispatch. CURRENT logs show bounded recurring 429/timeout reopen events on several secondary routes rather than a same-cycle sibling storm; healthy probes continue across all 9 currently effective providers. Circuit/provider/structured-route health therefore continues to filter degraded routes without changing durable priority or producing a routed-request failure in current telemetry.
7. **PARTIAL — canonical write-back / anti-drift.** The latest live-verified canonical product-code head is `fae89d7b4a91090c3a61bd361f2556315a830f0a`; it contains the provider-wide fast-failure budget fix plus the previously proven named-set/auth/provider-key/failure-domain behavior. PROD acceptance on that exact configured SHA is PASS. Remaining canonical anti-drift is test-source convergence only: the deterministic regression exists and passes in the live DEV owner/full suite, but canonical `test/test.js` is >200 KiB for the precise Git patch route and Coding Station writeback hit `ENOSPC`. Do not create a duplicate test owner to bypass the environment blocker.
8. **PARTIAL — exact-source identity split by environment.** PROD configured/deployed source identity is exact and accepted at `fae89d7b4a91090c3a61bd361f2556315a830f0a`. Permanent DEV is intentionally still an older source-layout/control runtime, so exact tested Git SHA == DEV filesystem identity remains **OPEN**. This DEV drift is rollback/debug-lane hygiene, not a PROD functional blocker.
9. **PASS — SourceLoop readiness for future live patches.** Runtime Target Registry revision `2` now provides the `fcm-dev` write-back mapping; the temporary mutation allowlist was restored exactly and a current-SHA no-write probe returned `target_registry_scope_denied`, independently proving privilege-window closure. Forward and reverse non-loaded test-file canaries both produced valid SourceLoop artifacts and restored the file to exact original SHA `e8461ef5be3f3736ff9796d440c6979b632ae79d0148332be00c49c8497c91c3`. No FCM reload/redeploy occurred. Historical pre-mapping journal entries remain historical and are not retroactively reconstructed.
10. **PASS — upstream/fork-drift classification gate.** Stock upstream `v0.5.81`, current upstream `0.5.88`/`2610ddaa...`, our canonical branch, and CURRENT runtime were compared before any further router mutation. The fast-coding provider-blocking incident is classified `ACCIDENTAL_REGRESSION`; upstream family-preserving cross-provider failover is `UPSTREAM_SUPERSEDED/ADOPT_CANDIDATE`; the `fcm:<set>` body resolver and structured-output contract are `CUSTOM_REQUIRED`. Until exact-source convergence is tested, this classification gate forbids speculative live rewrites.
...
- Legacy Gonka-only `fast-coding` set was retired in this historical batch. A bounded exhaustive pass tested 131 routeable keyed candidates and persisted 33 fresh chat-probe successes across 9 provider identities. At that time the set was ordered by the then-available SWE benchmark result and priority 1 was `llm7/minimax-m2.7`; this ordering is **superseded**. CURRENT durable order is Gonka DeepSeek priority 1, Gonka MiniMax priority 2, then the remaining 31 routes as reported by `/sets`. FCM does not run its own quality benchmark for this ordering.
...
### Batch 7 — Exact-source convergence candidate — REFERENCE ONLY / SUPERSEDED IMPLEMENTATION LANE
...
- An isolated Coding Station candidate was previously built at base `5b5d0e8614a5a80b0dc3cd56175ba5fd17e018f6` to study upstream family-failover convergence. It contained an upstream-derived `model-family.js`, `familyFailover` normalization, `pickNextCandidate()` router selection, `selection_reason` telemetry, and focused family/429 regressions. Syntax, standalone family tests (3/3), and `git diff --check` passed; router integration/full-suite validation never ran because dependencies were unavailable and `npm ci --ignore-scripts` hit `ENOSPC`. The repo session was then explicitly closed with `discard=true`; no candidate code remains published or retained as a writable implementation branch.
- **Latest user execution rule supersedes this as an implementation lane:** product code is edited/debugged directly in permanent `fcm-dev`, verified there, then written back to Git. The isolated candidate is therefore reference/research evidence only, not a source to publish, deploy, or use to bypass container-first debugging.
- If family-preserving failover becomes a concrete FCM requirement, re-derive the smallest live delta from CURRENT `/app` + upstream evidence, patch `fcm-dev` stale-safely, run focused/full live gates, then canonicalize only the proven delta. Do not redeploy merely to activate this old candidate.
...
- OpenClaw stored sessions/audit were inspected, then model topology was verified rather than inferred from successful runs. `main`, `devteam`, `devteam-verifier`, and `code-reviewer` are configured for direct Gonka models and do not currently expose an FCM model/provider entry.
- Therefore recent OpenClaw successes are not FCM acceptance evidence. One 90 s OpenClaw timeout was investigated and localized to the direct Gonka transport (`provider=gonka`, `proxy.gonka.gg/v1/chat/completions`), so it is also not an FCM regression.
...
- Post-soak status at the time of Batch 9 was 33 persisted routes / 9 configured providers, 8 effective providers, 21 probe-fresh healthy routes and 12 broken/hidden routes. **Superseded by CURRENT readback:** the broker has 8 effective providers, 13 probe-fresh healthy routes and 20 broken/hidden routes, with `requestsRouted=26`, `successCalls=26`, `errorCalls=0`, `inFlight=0`, `shuttingDown=false` after two fresh live canaries. The changing probe pool is upstream-health drift, not a durable-set mutation.
...
- Current acceptance readback (2026-09-07): exact running image is still `739a06a7b704b9898a8fdfd821e23d0d961d6298`; `/health` PASS; the 33-route `fast-coding` set self-restored from `FCM_BOOTSTRAP_CONFIG`; unauthenticated `/v1/chat/completions` correctly returns **401** with client-token enforcement active. However `effectiveProviderCount=0`: DEV provider secrets were application-local literals, not project-shared variables, so the initial `{{project.*}}` provider references had nothing to resolve.
- Secret anti-drift fix: created **production-environment-scoped** shared variables for exactly the nine provider keys used by the 33-route set, copying values internally from the DEV app through Coolify's own encrypted models without printing or committing credential values. PROD app refs now use `{{environment.KEY}}` for those nine providers; `FCM_CLIENT_TOKEN` remains the already-existing project-shared reference. Coolify app-env readback marks all ten references `is_shared=true`.
- Applying those resolved env refs requires a container recreate. Three subsequent deploy/restart attempts did **not** replace the healthy existing PROD container: deployment queue rows `jerovyhmfo5qnjtmjxandjn6`, `mivktx2pjqsggic0u0amnmiw`, and `xgslay5coku2gty72vnvzrsl` failed while the old container stayed healthy and carried no consumer traffic.
- Owning infrastructure diagnosis is now explicit: failed deployment logs show Docker/containerd build extraction failing with `no space left on device`. Hostinger metrics report ~101.84 GB used on a 102.4 GB VPS disk. This is a host Docker-storage blocker, not an FCM code failure.
- Storage blocker changed without destructive intervention: fresh Hostinger metrics on 2026-09-08 show disk usage dropped from ~95.7 GB to ~90.0 GB used, consistent with Coolify's enabled scheduled Docker cleanup (`force_docker_cleanup=true`, daily schedule, threshold 80%). No manual host prune/delete was executed, so running containers, volumes, databases, DEV, and current PROD were preserved.
- With space recovered, a new exact-SHA force deploy was queued as `duokiiknuaujdgnsjdvpu9vh` for commit `739a06a7b704b9898a8fdfd821e23d0d961d6298`. CURRENT queue readback is `in_progress`; no new error/no-space signal is present yet. The existing PROD still reads `running:healthy` until deployment readback proves replacement.
- Anti-drift rule: do not mutate FCM code or run manual cleanup while this deployment is unresolved. First determine its post-state, then continue acceptance from the actual running container. Do not treat queue acknowledgement as success.
- DoD remains: deployment completes → running image SHA = `739a06...` → `/health` PASS → 33 routes persist without manual POST → provider effectiveness is non-zero/coherent from environment-shared refs → unauthenticated request stays 401 → authenticated plain/schema/named-set + 8-turn cadence PASS → one more recreate repeats the same state. Only then mark PROD release-ready and consider cutover.

### Batch 19 — production recurrence / provider-wide fast-failure budget — ACTIVE

- BMAD `bmad-help`, `bmad-testarch-test-design`, and `bmad-review-edge-case-hunter` were applied after the production intervention trigger fired. Consumer cutover remains forbidden until this gate closes.
- Current PROD application `5llbaomtlttzsukhin6xgrgc` is still pinned/running on canonical SHA `396dbf3181af8ac6551fbf4058e302a3273d4270`, healthy, with 33 persisted routes, Gonka DeepSeek/MiniMax at priorities 1/2, `autoHeal=false`, 8 effective providers, and real bearer auth (`/v1/*` without token → 401). Authenticated plain text, strict `json_schema`, and explicit named-set canaries PASS on this runtime.
- Production recurrence is real: one authenticated eight-turn cadence returned only 5/8 successes with client-visible `All routed models failed`; subsequent retries with a temporary runtime-only `maxRetries=8` improved but did not eliminate the boundary (7/8 then 8/8, with an additional observed client-visible 503 in surrounding acceptance). Therefore raising retries is rejected as the durable fix and is not canonical configuration.
- Owning defect: the router's global `maxRetries=3` / four-attempt budget counted fast provider-wide failures (HTTP 429 quota domain and auth failures) exactly like timeout/5xx attempts. Several early providers can fail quickly and exhaust the budget even while a later provider remains usable.
- Fix was implemented **container-first in permanent `fcm-dev`**. Provider-wide 429/auth still block that provider immediately, but each such fast provider skip extends only the per-request candidate scan allowance; timeout/transport/retryable 5xx continue consuming the ordinary bounded retry budget. Total scanning remains capped by `candidates.length`.
- Live regression added in the current runtime: with `maxRetries=1`, provider 1 returns 429, provider 2 returns 429, provider 3 succeeds; the request must reach provider 3. After the live patch, router syntax PASS, full live `npm test` PASS, same-container reload PASS, and authenticated DEV live canary PASS. SourceLoop captured the live source/test deltas.
- Proven product-code source was written back after live verification. Current canonical product-code head is `fae89d7b4a91090c3a61bd361f2556315a830f0a`.
- Canonical regression-test write-back is **VALIDATION/WRITEBACK BLOCKED**, not product-code failure: the correct owner is existing `test/test.js` (>295 KiB); a fresh exact-source Coding Station session at `fae89d7...` can read the repo but applying the one-file regression patch failed `ENOSPC: no space left on device`. Do not create a duplicate sidecar test merely to bypass this environment blocker.
- CURRENT PROD still has the temporary runtime `maxRetries=8` from diagnosis and is not release-ready. The next release deploy must restore canonical seed `maxRetries=3` and deploy the proven fix, then prove recurrence closure without the workaround.
- Batch 19 closure: the functional recurrence gate is now **PASS**. PROD was deployed from exact source `fae89d7b4a91090c3a61bd361f2556315a830f0a`, then cleanly recreated again. Fresh post-recreate readback shows one healthy container on that exact image, 33 persisted routes, Gonka DeepSeek/MiniMax still priorities 1/2, `autoHeal=false`, 9 configured providers / 8 effective providers, `inFlight=0`, `shuttingDown=false`, and real bearer auth (`/v1/*` without token → 401). Authenticated plain text, strict `json_schema`, and explicit named-set canaries PASS.
...
### Batch 20 — historical consumer work — OUT OF CURRENT FCM SCOPE / SUPERSEDED

- Historical external-consumer operations are intentionally collapsed here so they cannot masquerade as current FCM DoD. They are not evidence for or against broker correctness and must not trigger FCM mutations.
- FCM-relevant durable result only: PROD has stable internal alias `fcm-prod-internal`, DEV has stable alias `fcm-dev-internal`, and PROD was cleanly recreated/accepted on exact product SHA `fae89d7b4a91090c3a61bd361f2556315a830f0a`.
- Any consumer endpoint wiring, consumer redeploy/restart behavior, AgentField status, or consumer black-box observation belongs outside this FCM SoT unless the user explicitly expands scope.
...
5. **Runtime gate is green.** CURRENT live router SHA-256 is `9a5b407869e9d7697ed9b6bcbabefb0523fa1642d4317c86ed608d18768db058`; final live `npm test` is 841 discovered / 837 PASS / 0 FAIL / 4 intentional skips across 160 suites. Same-container acceptance includes plain text, `json_schema`, eight-turn reviewer cadence, and fail-closed named-set selector behavior.
6. **Current routing state.** Persisted `fast-coding` contains 33 routes across 9 provider identities with Gonka DeepSeek priority 1 and Gonka MiniMax priority 2. `maxRetries=3`; `probeMode=balanced` is 120s. Route-local timeout/transport/retryable 5xx gets one same-provider sibling opportunity; provider-wide 429/auth crosses provider domains immediately.
7. **Canonical router head.** Latest live-verified canonical product-code head is `fae89d7b4a91090c3a61bd361f2556315a830f0a`; it includes sibling-aware failure domains, fail-closed `fcm:<set>` selection, env-backed provider-key fallback with config precedence, broker client bearer auth, and the provider-wide fast-failure budget fix proven in DEV then accepted on PROD.
8. **Upstream-first rule.** The live/fork base is 0.5.81 while current upstream is 0.5.88 at `2610ddaa8c20c75a61778dffd25c6562913ef6a3`. Before new router code, compare stock base, current upstream, canonical fork, and CURRENT runtime. Prefer upstream primitives when equivalent; keep custom code only for proven gaps/invariants.
9. **Keep vs adopt.** Keep the custom structured-output contract/atomic lifecycle/progressive repair and `fcm:<set>` body resolver. Treat upstream family-preserving cross-provider failover (`model-family.js` / `familyFailover`) as an adoption candidate for the next exact-source convergence batch, not as a reason for a speculative live rewrite.
10. **Remaining anti-drift.** Exact tested Git SHA == permanent DEV filesystem identity remains OPEN because the runtime is an older source layout. Canonical test convergence is PARTIAL: live-only `router-client-auth.test.js` has no current Git owner equivalent. The transient Coding Station family-failover candidate was unvalidated, then its repo session was explicitly closed/discarded; no candidate code is published or retained as an implementation branch. Under the latest execution rule future product-code work is container-first in `fcm-dev`. SourceLoop readiness for future live captures is PASS; canonical root `ERRORS.md` owns reusable environment/test-harness lessons.
11. **Next bounded move.** FCM-only scope. PROD is release-ready on exact live-verified canonical product SHA `fae89d7b4a91090c3a61bd361f2556315a830f0a`; DEV is healthy and remains rollback/control. No product-code task is open. The next ~30-minute batch starts only when a direct FCM request fails, a broker invariant breaks, a set becomes unusable, or the user defines an explicit FCM-only improvement DoD. Then: OBSERVE exact DEV runtime → patch smallest owner directly in `fcm-dev` → focused regression PASS → full live `npm test` PASS → same-runtime canary/readback PASS → canonical Git/SourceLoop write-back of only the proven delta. No consumer/AgentField/OpenClaw/OpenCode work, no GitHub-first programming, and no redeploy-as-debug.
