...
7. **PASS — verified canonical deltas.** The live-verified provider-aware probe fixes and regressions are written back and reread on `fix/fcm-long-stream-lifecycle`; current canonical code/test head before PLAN-only reconciliation is `60dc98c42897d498dff2e3ce2096df1875e6236c`. Canonical `router-daemon.js` now reuses the shared provider quota cooldown, honors probe 429 `Retry-After`, serializes startup probes per provider, excludes OPEN/paused routes from periodic probing, rechecks pause at dispatch, and preserves persisted router settings/probe cadence. Canonical `test/probe-cache.test.js` carries both OPEN→HALF_OPEN recovery and provider-429 sibling-suppression regressions. Exact-source full-suite validation remains blocked by the environment gate below; no canonical redeploy was performed.
