...
Status: ACTIVE — **FCM-only scope. Routing-quality reprioritization requested; product mutation blocked by a proven DEV live-patch bootstrap defect.** User-approved objective: durable tiers (1) Gonka first, (2) strongest proven free routes roughly DeepSeek-class, (3) paid RouterAI, (4) weaker free routes. Bounded health/circuit failover remains authoritative inside this durable preference policy; do not silently re-pin RouterAI to #1.
Last reconciled: 2026-09-12
Repository: `n0namer/free-coding-models`
Active working branch: `fix/fcm-long-stream-lifecycle`
Current deployed product-code head in DEV and PROD remains `6e17a250cd34469a14d575a45c20d0f2f6f8cfab`. CURRENT DEV evidence localizes the obsolete routing behavior to `src/core/config.js`, where env-derived RouterAI is constructed at priority 1 and normalized ahead of all routes. The typed mutation blocker is now root-caused: target `fcm-dev` is registered with `live_patch_runtime=node` and root `/app`, so VPS Terminal resolves `/app/live-aci-node-helper.mjs`; that helper file is absent in the FCM DEV container. The gateway then fails JSON parsing and reports `live_patch_invalid_output` even for READ and `node_check`. Direct/generic mutation remains correctly blocked by operator mediation, and changing the target registry to `live_patch_runtime=python` is currently denied by the server-owned registry allowlist (`target_registry_scope_denied`). PROD therefore intentionally remains on the previously accepted RouterAI-first order until the DEV control-plane path is restored and the new tier policy is proven container-first.
