...
4. **PASS — canonical live suite.** Fresh post-ranking `npm test` PASS: 831 tests discovered, 829 passed, 0 failed, 2 intentional live-canary skips, 159 suites, ~18.7 s. This suite includes the existing regressions that higher SWE score wins quality-focused scoring and that explicit router priority remains authoritative over latency/health tie-break scoring. Directly invoking `test/test.js` without the package auth-cleanup bootstrap produced expected 401 harness failures; rerunning through the package's canonical test entrypoint is green, so that direct invocation is a validation-environment artifact, not a router regression.
...
6. **PASS — SWE-first health-aware routing.** The 33 persisted routes are ordered by the available SWE benchmark score descending; tier is used only when SWE scores tie, then prior proven order is the final stable tie-break. The scores come from existing `sources.js` benchmark metadata; FCM does not run a separate model-quality benchmark. `probeMode=balanced` remains active at 30s and performs real chat-completion liveness probes; the circuit breaker removes unhealthy routes from the effective order without changing their durable SWE priority. Retryable provider failures skip the failed provider domain, while structured-schema failures remain route/role scoped.
