...
- An isolated Coding Station candidate was previously built at base `5b5d0e8614a5a80b0dc3cd56175ba5fd17e018f6` to study upstream family-failover convergence. It contains an upstream-derived `model-family.js`, `familyFailover` normalization, `pickNextCandidate()` router selection, `selection_reason` telemetry, and focused family/429 regressions. Syntax, standalone family tests (3/3), and `git diff --check` passed; router integration/full-suite validation never ran because dependencies were unavailable and `npm ci --ignore-scripts` hit `ENOSPC`.
