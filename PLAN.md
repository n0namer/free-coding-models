...
- **Functional FCM closure is DONE; 2 release-hygiene tasks remain non-blocking:**
  1. **Exact-source full-suite evidence — ENVIRONMENT-GATED MAINTENANCE.** The exact canonical branch currently cannot complete its normal dependency/test lifecycle in the available isolated runner because Coding Station fails before install/test with `ENOSPC`. This is not an FCM runtime failure and does not block CURRENT use. Do not deploy a future canonical release until an exact-source full suite is green.
