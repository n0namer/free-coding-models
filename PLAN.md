...
- DEV already has stable network alias `fcm-dev-internal`; PROD had no stable alias. To avoid binding any consumer to a deployment-specific container name, PROD application metadata was given the reversible alias `fcm-prod-internal` (rollback = restore alias to null). A force recreate on exact SHA `fae89d7...` was queued as `3juycwq7qfjmcpqnxscdufpe`; CURRENT queue state is still `in_progress`, while the previously accepted PROD container remains healthy. Do not change the shared `FCM_BASE_URL` until post-state proves the stable PROD alias resolves and `/health` passes through it.
- Planned endpoint cutover, once that alias gate passes: change the single shared server variable `FCM_BASE_URL` from `http://fcm-dev-internal:19280/v1` to `http://fcm-prod-internal:19280/v1`. Because all three consumer endpoint variables reference that owner, this is one reversible control-plane change. Exact rollback is restoring the old DEV URL; DEV must stay running throughout the observation window.
