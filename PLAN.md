...
- Legacy Gonka-only `fast-coding` set is retired. A bounded exhaustive pass tested 131 routeable keyed candidates and persisted all 33 fresh chat-probe successes across 9 provider identities, preserving DeepSeek as priority 1 while broadening the fallback pool.
...
6. **Multi-provider routing.** Persisted `fast-coding` now contains 33 fresh chat-probe-healthy routes discovered from 131 routeable keyed candidates across 9 provider identities. DeepSeek remains priority 1; failed/unproven candidates are excluded. Retryable provider failures can cross provider failure domains, while structured incompatibility is route/role scoped and is not inferred from generic chat liveness.
