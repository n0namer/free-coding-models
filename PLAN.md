...
4. **PASS — live package suite after scheduler fix.** Final `npm test` PASS: 832 tests discovered, 830 passed, 0 failed, 2 intentional live-canary skips, 159 suites, ~18.5 s. The new deterministic regression proves an unchanged probe schedule is not reset by repeated reschedule/config-reload requests and a real mode/interval change does replace the timer. An earlier unrelated benchmark performance threshold flake and an initial 5s-minimum test-assumption error were diagnosed and closed; the final unchanged product/test state is green.
...
6. **PASS — SWE-first + recurring real-inference health routing.** The 33 persisted routes remain ordered by existing `sources.js` SWE benchmark score descending; tier is only a tie-break, then prior proven order. FCM does not run its own quality benchmark. `probeMode=balanced` is 30s and uses real chat completions. The scheduler starvation defect is fixed: 10s config reloads no longer reset an unchanged 30s timer, and fresh post-reload logs show periodic probes at ~20:05:14, ~20:05:16, and ~20:05:20 after startup at ~20:04:44. Circuit/provider/structured-route health continues to filter unusable routes without changing durable SWE priority.
