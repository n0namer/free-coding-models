...
6. **PASS — Gonka-pinned head + provider-aware recurring real-inference health routing.** The 33 persisted routes are now explicitly headed by Gonka DeepSeek at priority 1 and Gonka MiniMax at priority 2; the remaining 31 routes preserve their prior relative SWE-ranked order. FCM does not run its own quality benchmark. `probeMode=balanced` now persists at 120s and still uses real chat completions. HTTP 429 pauses the provider/key domain using `Retry-After` when present, startup probes serialize siblings within each provider, periodic probes exclude OPEN routes until HALF_OPEN recovery and re-check provider pause at dispatch. Fresh post-reload logs show the expected recurring cycle starting ~120s after the 10:29:13Z restart (LLM7 at 10:31:13Z, Gonka at 10:31:37Z/10:31:49Z, then later healthy-provider probes) with no renewed OpenRouter/Requesty 429 storm. Circuit/provider/structured-route health continues to filter unusable routes without changing durable SWE priority.
