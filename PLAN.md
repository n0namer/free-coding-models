...
- Destructive cleanup gate: the lowest-risk recovery candidate is to free **Docker build cache / failed-build artifacts first**, preserving running application containers, volumes, databases, current DEV, and the healthy-but-not-release-ready PROD. Do not run host-wide prune, delete volumes, or remove running images. Because even builder-cache cleanup is destructive host maintenance, require explicit user confirmation before executing it.
- DoD remains: free enough build storage safely → recreate PROD from exact `739a06...` with environment-shared provider refs → `/health` PASS → 33 routes persist without manual POST → provider effectiveness is non-zero/coherent → unauthenticated request stays 401 → authenticated plain/schema/named-set + 8-turn cadence PASS → one more recreate repeats the same state. Only then mark PROD release-ready and consider cutover.
