...
- Final live-container package gate after the named-set edge hardening: `npm test` discovered 841 tests, 837 passed, 0 failed, 4 intentional skips, 160 suites, ~18.5 s. Authenticated black-box acceptance includes ordinary text PASS, `json_schema` PASS, an eight-turn reviewer-like `fast-coding` cadence PASS, and unknown `fcm:<set>` returning `404 set_not_found` instead of falling back to `activeSet`.
