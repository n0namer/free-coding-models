...
11. **Next bounded move.** Keep CURRENT `fcm-dev` as the acceptance control. Probe-pool health is degraded (13/33 fresh healthy, 20 broken/hidden) but request telemetry is 26/26 successful with 8 effective providers and no `All routed models failed`; do not patch merely to increase the healthy-count metric. Reopen code work only on a reproducible direct FCM defect or a separately justified product improvement. Any code change is container-first: observe CURRENT `/app`, stale-safe patch `fcm-dev`, run focused + full live gates, verify same-runtime behavior, then write back only the proven delta to Git. No GitHub-first programming and no redeploy-as-debug.
