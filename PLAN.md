...
5. **PARTIAL — post-reorder black-box availability.** The persisted 33-route `fast-coding` set spans 9 provider identities and the Gonka-first order survives the final same-container reload. The latest `response_format=json_schema` canary is PASS. The latest ordinary-text canary returned 503 because Gonka DeepSeek timed out, provider-domain failover skipped the sibling Gonka route, and the remaining bounded attempts hit 429s on LLM7/GoogleAI/OpenCode Zen. This does not invalidate the ordering change, but full post-reorder 2/2 live availability is not currently proven.
