...
- Course correction: `outreach-quality` must evolve into a broad dynamic pool of CURRENT contract-certified models. Top two eligible routes should use a race/hedge policy for availability/latency, while remaining certified routes provide scored/circuit-breaker fallback. A separate `outreach-judge` pool should prefer different provider/model families for failure-domain and bias independence. Outreach semantic/business validation remains outside FCM; FCM must not embed Outreach-specific `message_id` or evidence logic. Models qualify for these pools only through production-equivalent structured contract probes.
