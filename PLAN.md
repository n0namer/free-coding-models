...
- SourceLoop/FVE journal is CURRENT and records the live-patch chain, but the gap is now localized outside FCM behavior: `fcm-dev` is registered for `live_patch` without a SourceLoop `writeback` mapping, so its journal rows have `writeback={}` and `capture_envelope=null`; `sourceLoopAction artifact` therefore correctly cannot resolve an artifact. SourceLoop artifact export itself is proven healthy on the mapped `vps-terminal-dev-gateway` target. CURRENT FCM container image tag `krhkfc6xjtreidxxbf8xdia3:cd64d76cb6e9c7ede9c7ce556b786e8732a4a81e` is an exact Git commit in this repository, so `container_image_tag_sha` is viable. A typed registry upsert was attempted stale-safe and rejected before write as `target_registry_scope_denied`; FCM runtime/config remained unchanged. Treat this as a **SOURCELOOP_GAP / control-plane authorization blocker**, not an FCM application failure.
