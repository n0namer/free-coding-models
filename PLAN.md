...
4. **PASS — live package suite after provider-aware probe fix.** Final `npm test` PASS: 836 tests discovered, 834 passed, 0 failed, 2 intentional live-canary skips, 160 suites, ~18.3 s. Deterministic scheduler regressions now cover unchanged-timer survival, OPEN-route suppression with HALF_OPEN recovery after cooldown, and provider-level 429 pause suppressing sibling probes. Syntax checks for the live router and probe tests are PASS.
