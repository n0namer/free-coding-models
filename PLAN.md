...
7. **PASS — verified canonical deltas.** The live-verified provider-domain routing, SWE-first set state, and periodic-probe scheduler fix are written back and reread on `fix/fcm-long-stream-lifecycle`. Current canonical code/test head before PLAN-only reconciliation is `60cbba2ffe2650c81cc86ee9a20f471a5d49ddd5`; exact-source `node --check src/core/router-daemon.js` at the scheduler code head passed in Coding Station. The canonical probe scheduler regression is durable in `test/probe-cache.test.js`.
