...
- Fresh post-reload config readback confirms the Gonka-only set is removed. Persisted `fast-coding` now has four explicit routes across three provider failure domains: `gonka/deepseek-ai/DeepSeek-V4-Flash-0731` priority 1, `gonka/MiniMaxAI/MiniMax-M2.7` priority 2, `kilo/kilo-auto/free` priority 3, and `llm7/minimax-m2.7` priority 4. The migration is narrow: only legacy `fast-coding` sets whose existing models are all Gonka are expanded, using enabled routeable independent providers. Live `npm test` after this change is 829 PASS / 0 FAIL / 2 intentional skips across 159 suites; after same-container reload, live plain-text and `json_schema` canaries are 2/2 PASS.
...
5. **PASS — real black-box semantics.** After same-container reload, live ordinary text and `response_format=json_schema` canaries are 2/2 PASS with the persisted `fast-coding` set spanning Gonka, Kilo, and LLM7.
6. **PASS — provider compatibility / failure-domain routing.** Structured requests keep immutable acceptance semantics; retryable provider failures skip the failed provider domain; structured-schema failures are route/role scoped rather than globally disabling plain text.
...
6. **Multi-provider routing.** Persisted `fast-coding` is no longer Gonka-only: Gonka DeepSeek priority 1, Gonka MiniMax priority 2, Kilo Auto Free priority 3, LLM7 MiniMax priority 4. Retryable provider failures can cross provider failure domains; structured incompatibility is route/role scoped.
