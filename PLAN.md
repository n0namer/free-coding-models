...
- **Current consumer-orchestration lane — partially observable, still non-blocking.** AgentField capability discovery continues to return `Bad Gateway`, so that lane is unavailable for a clean managed replay. OpenClaw itself is observable: recent `main` sessions include successful completed runs through Gonka/MiniMax, while one 90 s timeout was independently localized to OpenClaw's direct provider path (`provider=gonka`, `url=https://proxy.gonka.gg/v1/chat/completions`) rather than the FCM endpoint. That timeout therefore does **not** reopen the FCM incident.
