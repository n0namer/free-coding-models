...
- CURRENT `outreach-quality` is now a six-route Gonka-first primary/recommendation set: (1) `gonka/deepseek-ai/DeepSeek-V4-Flash-0731`, (2) `gonka/MiniMaxAI/MiniMax-M2.7`, then `codestral/codestral-2508` → `groq/openai/gpt-oss-120b` → `groq/openai/gpt-oss-20b` → `llm7/codestral-latest`. CURRENT `outreach-judge` is a separate non-Gonka pool: `codestral/codestral-2508` → `groq/openai/gpt-oss-120b` → `groq/openai/gpt-oss-20b` → `llm7/codestral-latest`. Both virtual models are exposed by `/v1/models`; `fast-coding` remains the verified 20-route Gonka-first contour.
- Fresh 2026-09-09 inventory: `/api/models` exposes 206 catalog models; 116 model routes are credentialed; only 7 were `status=up` at inventory time: `googleai/gemini-3-flash-preview`, `googleai/gemini-3.1-flash-lite`, `codestral/codestral-2508`, `zai/zai/glm-4.5-flash`, `llm7/minimax-m2.7`, `gonka/MiniMaxAI/MiniMax-M2.7`, `gonka/deepseek-ai/DeepSeek-V4-Flash-0731`. Pool breadth is therefore dynamic, but Gonka priorities 1–2 are architectural invariants.
