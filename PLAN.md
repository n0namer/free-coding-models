...
- Live router config is now intentionally constrained to **Gonka Proxy only**, active set `fast-coding`, with exactly two routes: `gonka/MiniMaxAI/MiniMax-M2.7` priority 1 and `gonka/deepseek-ai/DeepSeek-V4-Flash-0731` priority 2. All previous Cloudflare/Ollama/Qwen/Google/NVIDIA entries were removed from the active set through the running FCM router API, which persisted the change to `/config/config.json` without redeploy.
- Real black-box generation canary against the actual daemon now PASS: HTTP 200, routed model `gonka/MiniMaxAI/MiniMax-M2.7`. This closes the previous external-quota blocker and proves the live priority-1 Gonka route is callable with current credentials.
...
6. **Live provider routing is now intentionally Gonka-only.** Active set `fast-coding` contains exactly `gonka/MiniMaxAI/MiniMax-M2.7` priority 1 and `gonka/deepseek-ai/DeepSeek-V4-Flash-0731` priority 2. A real black-box chat request returned HTTP 200 through priority-1 MiniMax, so the prior quota blocker is closed.
