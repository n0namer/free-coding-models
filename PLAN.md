...
**P2.1 FCM PROD RELEASE-READY / FCM DEV ROLLBACK-READY — FUNCTIONAL GATE DONE; NO ACTIVE PRODUCT-CODE TASK.**

Active work is FCM-only. Consumer integration/cutover is outside this SoT's current execution scope and is not a release gate for FCM. The next code batch starts only on a direct FCM intervention trigger or a separately explicit FCM product-improvement DoD. Canonical regression-test write-back remains environment-gated maintenance.
...
7. **PARTIAL — canonical write-back / anti-drift.** The live-verified router logic is now converged into canonical `fix/fcm-long-stream-lifecycle` without redeploy: sibling-aware request routing is present, HTTP 429 is classified as provider-wide for both non-stream and pre-commit streaming paths, and OpenAI-compatible `model=fcm:<set>` now resolves to the requested named set instead of silently falling back to `activeSet`. Canonical router write-back commits are `59525bfc9f813a420e0b3cc3cc43180ae7315b3e`, `9930b57fb90a010e05e1a1199eaaf234b06db14e`, `cdb33548dd7b21fbcaa8b4fa7a9ad083c7da6df2`, `0efc0ccbc4300efa8d3bda95c43f056b8c846c56`, and fail-closed edge hardening `382d9606268803cc86dfced5f8f639b737345457`. Runtime proof is already green: deterministic named-set routing PASS, same-container restart PASS, live `model=fcm:<set>` negative black-box smoke PASS, and the full live package `npm test` exits 0. Remaining anti-drift is test-source convergence only: the old-runtime `test/router-client-auth.test.js` does not exist in current Git layout, canonical `test/test.js` is the correct owner but is >200 KiB for the precise Git patch route, and Coding Station remains blocked on writes by `ENOSPC: no space left on device` even after session GC. Do not create a duplicate test file solely to bypass that environment blocker. No canonical redeploy was performed.
...
- 20/80 rule for this batch: no more load generation or code changes for reassurance. Highest-value next move is topology readback only: identify actual FCM consumers, if any. If none exist, there is nothing to cut over and PROD can remain release-ready/idle until a consumer is intentionally onboarded.
- Cutover preflight found one current Coolify application whose resolved LLM endpoint is FCM DEV: `universal-solver-agentfield-exact-dev-git-20260825` (`edshqtkwskg3lrczekhcmd71`). Its `FCM_BASE_URL`, `LLM_BROKER_BASE_URL`, and `PR_AF_LLM_ENDPOINT` all resolve through the same shared server variable to `http://fcm-dev-internal:19280/v1`; matching API-key variables are present/shared. No other application reference to the DEV FCM endpoint was found in the bounded application/shared-variable scan.
- DEV already has stable network alias `fcm-dev-internal`; PROD had no stable alias. PROD application metadata was given reversible alias `fcm-prod-internal` (rollback = restore alias to null), then cleanly recreated on exact SHA `fae89d7...` via deployment `3juycwq7qfjmcpqnxscdufpe`. Deployment finished PASS and fresh `http://fcm-prod-internal:19280/health` returned healthy with 33 models / 9 providers / 8 effective providers / `autoHeal=false`. Stable PROD endpoint gate is therefore PASS.
- Cutover owner is the single shared server variable `FCM_BASE_URL`; all three consumer endpoint vars inherit it. The first restart-only path was correctly rejected after two failures: Coolify `just_restart()` prepares a helper, then for this Docker Compose application fails to find a single production image, flips `restart_only=false`, re-enters full deployment, and tries to create the same helper name again. This explains the observed helper-container name conflicts and makes identical restart retries the wrong recovery path.
- New evidence changed strategy: the stale conflicting helper containers were already gone, so the shared owner was switched again to `http://fcm-prod-internal:19280/v1` and the consumer was redeployed through the normal full `StartApplication` path instead of `RestartApplication`. Deployment `bkaah9dgg9ppbj2dukzsdspe` finished successfully on existing consumer source SHA `ffa6c6a56814b59da19903bd56c04f8cafdb44ae` without any GitHub-first code change.
...
11. **Next bounded move.** FCM PROD is release-ready on exact `fae89d7...`; the sole discovered consumer has been full-redeployed successfully after the shared endpoint owner was switched to `http://fcm-prod-internal:19280/v1`. Its workforce/control-plane/deep-research/postgres/diagnostics containers are running and healthy; DEV remains running as rollback. No product-code task is open. The next ~30-minute batch is post-cutover observation only: wait for AgentField capability/execution gateway recovery, run one bounded consumer execution, and require PROD `requestsRouted` to increase with successful completion. If the AgentField gateway continues returning `Bad Gateway`, keep the applied cutover and DEV rollback, record EVIDENCE_MISSING for consumer black-box proof, and do not mutate FCM to compensate.
