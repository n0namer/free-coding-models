...
CURRENT live runtime has progressive `json_schema` repair, provider-failure-domain failover, and a persisted 33-route `fast-coding` pool across 9 provider identities. User routing policy now overrides the previous pure SWE-first order at the head of the cascade: `gonka/deepseek-ai/DeepSeek-V4-Flash-0731` is priority 1, `gonka/MiniMaxAI/MiniMax-M2.7` is priority 2, and every remaining route keeps its prior relative order starting with `llm7/minimax-m2.7` at priority 3. Provider-aware real-inference health behavior remains unchanged: HTTP 429 honors `Retry-After` through the shared provider cooldown, scheduled probes skip OPEN routes until HALF_OPEN recovery and skip paused providers again at dispatch, startup bursts serialize models within each provider, persisted router settings survive reloads, and balanced cadence is 120s. The reorder was persisted through the app's normal config-save path, the temporary migration code was removed byte-exact, and a same-container reload confirmed priorities 1/2 remain Gonka. A post-reorder LIVE canary is mixed: `json_schema` PASS, while plain text returned 503 after Gonka DeepSeek timed out and the bounded retry budget then encountered 429s on the next independent providers. This is runtime availability evidence, not an ordering failure. Also, `All routed models failed` does not mean all 33 routes were attempted: current `maxRetries=3` bounds one request to four attempts, and provider-domain failures can skip sibling models from the same provider.
