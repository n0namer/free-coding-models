...
9. **BLOCKED — SourceLoop target registration, not artifact engine.** FVE journal is readable; mapped-target artifact export is separately proven PASS. `fcm-dev` lacks the target-registry `writeback` mapping needed to create a capture envelope, and the safe typed upsert is currently rejected before write as `target_registry_scope_denied`. No FCM state changed. Closure requires an explicitly authorized one-target registry mutation window, immediate restoration/closure proof, and a reversible non-loaded test-file capture/artifact canary with exact SHA restoration.
