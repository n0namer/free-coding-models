...
- Batch 19 DoD: obtain a safe canonical test-owner write-back path or explicitly record the environment blocker without duplicating ownership → deploy exact proven source with canonical `maxRetries=3` → `/health`, 33-route Gonka-first set, provider/auth/plain/schema/named PASS → fresh eight-turn cadence 8/8 with no client-visible exhaustion → one more clean recreate → repeat identity/routes/auth/provider/cadence readback. Only then mark PROD release-ready; DEV remains rollback/control until cutover observation completes.
