...
11. **Next bounded move.** Keep CURRENT `fcm-dev` as the acceptance control. Latest readback is 12/33 fresh healthy routes, 21 broken/hidden, **9/9 effective providers**, and 27/27 successful routed calls with 0 telemetry errors. Do not patch merely to increase the healthy-route metric. Reopen code work only when the Batch 12 intervention trigger fires or when a separately justified product improvement has a concrete DoD. Any code change is container-first: observe CURRENT `/app`, stale-safe patch `fcm-dev`, run focused + full live gates, verify same-runtime behavior, then write back only the proven delta to Git. No GitHub-first programming and no redeploy-as-debug.
