...
- The CURRENT branch head is the commit containing this SoT; do not hard-code it here because every PLAN reconciliation advances that value. The latest live-verified product-code head remains `382d9606268803cc86dfced5f8f639b737345457`; subsequent reconciliation commits are documentation/operational-ledger updates unless separately marked as live-verified code.
...
- Cutover owner is the single shared server variable `FCM_BASE_URL`; all three consumer endpoint vars inherit it. A first cutover attempt changed that owner to `http://fcm-prod-internal:19280/v1`, then tried to restart the sole discovered consumer. Two bounded restart attempts (`ocdggoxh3meoucmefnzg4wac`, `wdsnistl4car1kzbd0f5vefo`) failed before touching the consumer workload because Coolify helper startup hit a container-name conflict. Existing consumer containers remained running/healthy on their old runtime.
- Recovery was applied immediately: shared `FCM_BASE_URL` was restored exactly to `http://fcm-dev-internal:19280/v1`, so an unrelated future consumer restart cannot silently cut over. DEV remains running and current consumer state remains on DEV.
- Cutover is now **BLOCKED on consumer restart/recreate control-plane behavior, not on FCM PROD**. Do not retry the same Coolify restart path again without new evidence. Next safe move is to identify a supported no-drift way to recreate only that consumer with the updated shared env (or fix the helper-name conflict at the Coolify layer), then repeat one reversible endpoint switch + readback. PROD itself stays release-ready.
...
7. **Canonical router head.** Latest live-verified canonical router code head before PLAN-only commits is `382d9606268803cc86dfced5f8f639b737345457`; it includes sibling-aware failure domains plus fail-closed `fcm:<set>` model selection.
...
11. **Next bounded move.** Keep CURRENT `fcm-dev` as the acceptance control. Latest CURRENT readback is `requestsRouted=35`, `successCalls=35`, `errorCalls=2` with both errors absorbed Gonka `http_429`, **9/9 effective providers**, 12/33 probe-fresh healthy routes, and no terminal route-exhaustion signal; the fresh full live `npm test` and eight-turn cadence remain PASS. **No implementation task is currently open.** Reopen code work only when the Batch 12 intervention trigger fires or a separately justified product improvement has an explicit DoD. Then work in one ~30-minute container-first batch: observe CURRENT `/app` → stale-safe patch `fcm-dev` → focused tests → full live gate → same-runtime readback/canary → canonical write-back of only the proven delta. No GitHub-first programming and no redeploy-as-debug.
