...
CURRENT live runtime has progressive `json_schema` repair, a persisted 33-route `fast-coding` pool across 9 provider identities, and Gonka pinned at priorities 1/2. The post-reorder 503 was traced to an FCM routing defect rather than SWE runtime failure: on the first retryable provider-domain failure (`timeout`/retryable 5xx/429), the request-local router immediately blocked the whole provider, so a timeout on `gonka/deepseek-ai/DeepSeek-V4-Flash-0731` skipped the priority-2 sibling `gonka/MiniMaxAI/MiniMax-M2.7` and spent the remaining bounded attempts on independent providers that could then return 429/400. The permanent `fcm-dev` container is now patched container-first so the first provider-domain failure preferentially receives exactly one untried sibling attempt from the same provider; only a second provider-domain failure (or absence of a sibling) blocks that provider for the request. Auth failures remain provider-wide immediately. The global retry budget remains unchanged: `maxRetries=3` means at most four model attempts. A deterministic regression, `tries one same-provider sibling before crossing providers on provider-domain failure`, is PASS; router syntax is PASS; the same container was restarted without rebuild/redeploy and returned healthy with `activeSet=fast-coding`, `activeModelCount=33`, and `activeProviderCount=9`. The authoritative runtime config is `/config/config.json` and currently persists `maxRetries=3`, `requestTimeoutMs=60000`, and `streamStallTimeoutMs=30000`; the stale `/root/.free-coding-models.json` is not the daemon config SoT. A fresh authenticated post-fix multi-turn OpenAI-compatible canary is still required before claiming end-to-end cadence DONE because the current operator surface blocks a secret-bearing direct `/v1/chat/completions` smoke; unauthenticated access correctly returns 401.
