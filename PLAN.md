...
10. **Remaining anti-drift.** Exact tested Git SHA == permanent DEV filesystem identity remains OPEN because the runtime is an older source layout. Canonical test convergence is also PARTIAL: live-only `router-client-auth.test.js` has no current Git owner equivalent; CURRENT Coding Station health/readiness is green but repo-session creation returned `Gateway Timeout` twice, while earlier `ENOSPC` is historical. SourceLoop readiness for future captures is PASS. Canonical root `ERRORS.md` now exists with verified reusable lessons for the test-harness auth mismatch, runtime config SoT, and exact-source runner failure handling.
11. **Next bounded move.** The CURRENT live FCM itself is the control and its functional/reliability gate is DONE. Consumer integration is not part of the objective. Do not mutate, restart, or load-test the broker further without a new FCM-layer signal; the latest repeated soak is 24/24 PASS with 0 telemetry errors while the route pool is partially degraded. Reopen runtime debugging only for a reproducible direct FCM failure (`/v1/chat/completions`, named-set selection, structured contract, failover, lifecycle, health/routing telemetry). Batch 7 exact-source/upstream convergence remains separate non-blocking release maintenance when writable workspace capacity exists.
